...
function createElement(type, props) {
    ...
    Object.keys(props).forEach((key)=>{
        if (key === "dataset") Object.keys(props[key]).forEach((dataKey)=>{
...
table.addEventListener("click", (e)=>{
    ...
    for(let i = 0; i < rows.length; i++)for(let cell = 0; cell < table.rows[i + 1].cells.length; cell++)table.rows[i + 1].cells[cell].innerHTML = rows[i][cell];
});
form.addEventListener("click", (e)=>{
    ...
    const userData = {};
    for (const [key, value] of formData.entries())if (key === "Salary") userData[key] = "$" + Number(value).toLocaleString();
    ...
    if (validateForm(userData)) {
        button.type = "reset";
        table.rows[table.rows.length - 2].insertAdjacentHTML("afterend", `<tr><td>${userData["Name"]}</td><td>${userData["Position"]}</td><td>${userData["Office"]}</td><td>${userData["Age"]}</td><td>${userData["Salary"]}</td></tr>`);
    }
});
